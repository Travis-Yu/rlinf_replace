...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def _read_joint_states(self):
        ...
        try:
            while not self._stop_thread.is_set():
                ...
                with self._lock:
                    ...
                    if dxl_comm_result != COMM_SUCCESS:
                        ...
                        if consecutive_comm_failures >= 50:
                            raise RuntimeError(
                                "50 consecutive Dynamixel sync-read failures "
                                f"(last communication result={dxl_comm_result})"
                            )
                        continue
                    consecutive_comm_failures = 0
                    for i, dxl_id in enumerate(self._ids):
                        ...
                        if self._groupSyncRead.isAvailable(
                            ...
                            raise RuntimeError(
                                "Failed to get velocity for Dynamixel with "
                                f"ID {dxl_id}"
                            )
                        # position
                        if self._groupSyncRead.isAvailable(
                            ...
                            raise RuntimeError(
                                "Failed to get joint angles for Dynamixel with "
                                f"ID {dxl_id}"
                            )
            ...
            print(f"Dynamixel reader stopped: {exc}", flush=True)
    ...
    def get_positions_and_velocities(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        while self._joint_angles is None or self._velocities is None:
            if self._read_error is not None:
                raise RuntimeError("Dynamixel reader failed") from self._read_error
            time.sleep(0.1)
        if self._read_error is not None:
            raise RuntimeError("Dynamixel reader failed") from self._read_error
    ...
    def _check_port_availability(self) -> bool:
        ...
        try:
            ...
            if shutil.which("lsof") is None:
                print("lsof is not installed; skipping serial-port ownership check")
                return True
    ...
    def _kill_processes_using_port(self) -> bool:
        ...
        try:
            if shutil.which("fuser") is None:
                print("fuser is not installed; cannot release a busy serial port")
                return False
            result = subprocess.run(
